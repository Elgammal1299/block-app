...
class FocusModesGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FocusSessionCubit, FocusSessionState>(
      builder: (context, state) {
        final activeFocusMode = state is FocusSessionActive
            ? _getActiveFocusModeType(state)
            : null;
  ...
  FocusModeType? _getActiveFocusModeType(FocusSessionActive state) {
    ...
    if (duration == 60) return FocusModeType.meTime; // 1 hour
    if (duration == 60) return FocusModeType.deepWork; // 1 hour
